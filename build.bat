...
if "%BUILD_TYPE%" == "debug" (
    cmd /c "%PROJECT_BASE%deps\boost\tools\jam\src\bin.ntx86\bjam" --toolset=msvc --with-date_time --with-regex --with-thread --with-system --with-program_options variant=debug link=static threading=multi >NULL
)

if "%BUILD_TYPE%" == "release" (
    cmd /c "%PROJECT_BASE%deps\boost\tools\jam\src\bin.ntx86\bjam" --toolset=msvc --with-date_time --with-regex --with-thread --with-system --with-program_options variant=release link=static threading=multi >NULL
)

if "%BUILD_TYPE%" == "all" (
    cmd /c "%PROJECT_BASE%deps\boost\tools\jam\src\bin.ntx86\bjam" --toolset=msvc --with-date_time --with-regex --with-thread --with-system --with-program_options variant=debug,release link=static threading=multi >NULL
)
...
if exist "%PROJECT_BASE%build-aux\sandbox-tests.xml" del /S /Q "%PROJECT_BASE%build-aux\sandbox_%MSVC_VERSION%-tests.xml" >NUL

if "%BUILD_TYPE%" == "debug" (
    "%MSBUILD%" "%PROJECT_BASE%sandbox.sln" /t:%REBUILD% /p:Platform=Win32,Configuration=Debug,VCBuildAdditionalOptions="/useenv"
    if errorlevel 1 exit /b 1
    if exist "%PROJECT_BASE%*.cache" del /S /Q "%PROJECT_BASE%*.cache" >NUL
)

if "%BUILD_TYPE%" == "release" (
    "%MSBUILD%" "%PROJECT_BASE%sandbox.sln" /t:%REBUILD% /p:Platform=Win32,Configuration=Release,VCBuildAdditionalOptions="/useenv"
    if errorlevel 1 exit /b 1
    if exist "%PROJECT_BASE%*.cache" del /S /Q "%PROJECT_BASE%*.cache" >NUL
)
