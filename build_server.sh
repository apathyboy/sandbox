...
version=3.0.0       # mmoserver project version
...
if [ ! -d $basedir/deps ]; then
    filename=sandbox-deps-$version.tar.bz2
    
    # Look for the dependencies source file and download if missing
    if [ ! -f $basedir/$filename ]; then
        wget -nc --no-check-certificate https://github.com/downloads/apathyboy/sandbox/$filename
    fi
...
if [ "Darwin" == "$os" ]; then
    export DYLD_LIBRARY_PATH=$basedir/deps/mysql/lib:$basedir/deps/mysql-connector-cpp/lib:$basedir/deps/tbb/lib
fi
