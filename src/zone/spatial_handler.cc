...
void HandlePositionUpdate(shared::Session& session,
...
void HandleSit(shared::Session& session, shared::ByteBuffer& message) {
  ...
  session.sendHardcodedPacket(
    "packets/Spatial/SelfPostureUpdate_Sit.txt", false);
  session.sendHardcodedPacket(
    "packets/Spatial/PostureUpdate_Sit.txt", false);
}
...
void HandleStand(shared::Session& session, shared::ByteBuffer& message) {
  ...
  session.sendHardcodedPacket(
    "packets/Spatial/SelfPostureUpdate_Stand.txt", false);
  session.sendHardcodedPacket(
    "packets/Spatial/PostureUpdate_Stand.txt", false);
}
...
void HandleProne(shared::Session& session, shared::ByteBuffer& message) {
  ...
  session.sendHardcodedPacket(
    "packets/Spatial/SelfPostureUpdate_Prone.txt", false);
  session.sendHardcodedPacket(
    "packets/Spatial/PostureUpdate_Prone.txt", false);
}
...
void HandleKneel(shared::Session& session, shared::ByteBuffer& message) {
  ...
  session.sendHardcodedPacket(
    "packets/Spatial/SelfPostureUpdate_Kneel.txt", false);
  session.sendHardcodedPacket(
    "packets/Spatial/PostureUpdate_Kneel.txt", false);
}
...
void HandleSpatialChat(shared::Session& session, shared::ByteBuffer& message) {
  ...
  std::wstring text(
    ...
    reinterpret_cast<const wchar_t*>(
      &packet_data[message.readPosition() + ((textsize-moodLength) * 2)]));
